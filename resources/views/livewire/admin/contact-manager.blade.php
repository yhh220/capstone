    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition {{ !$msg->is_read ? 'font-semibold bg-yellow-50/50' : '' }}">
                    <td class="px-4 py-3">
                        <div class="font-semibold text-gray-800">{{ $msg->name }}</div>
                        <div class="text-xs text-gray-400">{{ $msg->email }}</div>
                    </td>
                    <td class="px-4 py-3 hidden md:table-cell text-gray-600">{{ Str::limit($msg->subject, 40) }}</td>
                    <td class="px-4 py-3 hidden lg:table-cell text-gray-400 text-xs">{{ $msg->created_at->format('d M Y') }}</td>
                    <td class="px-4 py-3">
                        <span class="text-xs px-2 py-0.5 rounded-full font-semibold {{ !$msg->is_read ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500' }}">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button wire:click="viewMessage({{ $msg->id }})"
                                    class="text-blue-600 hover:text-blue-800 font-medium text-xs px-2 py-1 rounded hover:bg-blue-50 transition">View</button>

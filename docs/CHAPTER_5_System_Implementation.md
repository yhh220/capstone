...
The standout feature of the detail experience is the **three-dimensional product configurator**, built with **Three.js**. For products that support it, the customer can view and rotate a 3D model of the accessory directly in the browser. The models are loaded using Three.js's `GLTFLoader` and are aggressively optimised for the web: **Draco** geometry compression, textures downscaled and re-encoded as **WebP**, and a conservative mesh simplification — together these took the main car model from 24.5 MB down to **3.0 MB (an 87% reduction)** with the part names and door animations verified intact, so the interactive model loads quickly even on a mobile connection. The configurator runs entirely client-side once loaded, giving the customer a smooth, immediate way to inspect a product from every angle before buying, a strong differentiator for a car-accessories showroom where fit and appearance matter.

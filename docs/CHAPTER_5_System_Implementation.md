...
- **Staff (operational worker)** — the day-to-day floor role: confirming bookings, marking orders paid, shipped, and delivered, cancelling orders and processing refunds, importing product and order data, creating and editing products, curating homepage testimonials, and working the contact-message inbox. Staff can never **delete anything**, never **export** data (bulk exfiltration stays admin-only), and never manage user accounts.

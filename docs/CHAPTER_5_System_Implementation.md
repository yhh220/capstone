...
The **Customer** resource lists registered customers and, because every order carries the customer's `user_id`, shows each customer's order count and lets the owner drill into that customer's orders. The customer is always identified by their user account, which is the reliable link between a person and their purchases. The existing **Feedback** resource remains the owner-curated homepage testimonial tool. Separately, the **Product Reviews** resource lets both staff and administrators inspect, edit, delete, and directly toggle the storefront visibility of verified-purchase reviews from the listing table. This distinction prevents marketing testimonials from being confused with product-specific customer reviews.
...
- **Staff (operational worker)** — the day-to-day floor role: opening order details, confirming, editing, and rescheduling bookings, marking orders paid, shipped, and delivered, cancelling orders and processing refunds, importing product and order data, creating and editing products, curating homepage testimonials, and working the contact-message inbox. Staff can never **delete anything**, never **export** data (bulk exfiltration stays admin-only), and never manage user accounts.

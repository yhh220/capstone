...
On the server side, the filters are composed into a single database query rather than being applied in PHP after the fact, so that only the products actually needed for the current page are fetched. In-stock products are ordered before zero-stock products; within each availability group, genuine discounted products (`sale_price` lower than the regular price) are ordered before full-price products, then each group is ordered newest first. The same priority is used in the homepage Featured Products section so current, purchasable promotions are immediately visible. Pagination limits each request to a manageable number of products, and the search matches against the localised product name so that customers searching in Malay or Chinese (Section 5.11) find the same products a customer searching in English would.
...
Each product has a dedicated detail page that presents its image gallery, localised name and description, price, stock availability, and an add-to-cart control with a quantity selector. Product images are managed through the **Spatie Media Library** package, which stores each upload, associates it with the product, and generates the responsive image sizes used across the site. The catalogue's first image remains the primary image used by cards and SEO; the product page then exposes the full ordered gallery. The owner can upload several JPEG, PNG, or WebP images and drag them into order in the product form. On the storefront, a responsive main image is paired with horizontal, touch-scrollable thumbnails, previous/next controls, a position indicator, and keyboard left/right support. This keeps the experience usable on narrow phone screens without loading a separate gallery library.

The detail page also includes **product-specific customer reviews**. A review is linked to both a customer account and a product, and may only be submitted after that customer has an order containing the product in the completed (`delivered`) state. This provides a meaningful “verified purchase” signal while preventing anonymous spam. A new verified-purchase review is published immediately; staff or an administrator can subsequently edit, delete, or directly switch its storefront visibility on or off. The page shows the average visible rating, total review count, rating stars, dates, and review text, while the author sees a clear explanation if they are not yet eligible to review. To prevent a long review history from extending the product page indefinitely, it shows five visible reviews at a time with newest, highest-rating, and lowest-rating sorting and Previous/Next navigation.
...
**Delivery is limited to the area the shop actually serves.** The address form's state picker offers only the West Malaysian states — Sabah, Sarawak, and Labuan are excluded with an explanatory note, since the business does not ship to East Malaysia — and the same list is enforced server-side as a validation whitelist, so a crafted request cannot place an order to an address the shop cannot fulfil. A customer's previously saved East-Malaysian address still displays correctly; it simply cannot be chosen for a new order.
...
**Store pickup is a first-class alternative to delivery.** Because the business is fundamentally a walk-in showroom, checkout opens with a choice between courier delivery and **free self-collection at the Shah Alam showroom**. Choosing pickup removes the address fields entirely (replaced by a card showing the pickup location), waives the shipping fee, and records the order's fulfilment method. The choice is whitelisted server-side, so a crafted request cannot invent a fee-free fulfilment mode: anything other than the two known methods falls back to delivery and its full address validation.

Pickup customers must also choose a **collection date and time** before continuing to payment. The selectable dates cover the next thirty days and exclude the same closed weekdays configured for the showroom. Time options are generated from the same business-start, business-end, and slot-length settings used by the appointment calendar, with past same-day times removed. This deliberately reuses the operating-hours source of truth but does **not** consume service-booking capacity: collecting a packed order is not an installation appointment. The selected `pickup_at` value is revalidated on the server immediately before the order transaction is created, so a crafted request, stale tab, or changed business hour cannot save an invalid collection time.

Downstream, every surface adapts to a pickup order — the payment page and invoice show "store pickup" instead of a shipping line, the "shipped" lifecycle stage is presented to the customer as **"Ready for Pickup"** (with a collection email naming the showroom address instead of a tracking number), and "delivered" reads as "Collected". The selected collection time appears in the customer account, order tracker, confirmation and ready-for-pickup emails, and the admin order record. Before collection, staff can use a guarded **Reschedule Pickup** action; it applies the same opening-hours validation and emails the customer the revised time.

*[Figure 5.7a: The checkout fulfilment choice — courier delivery or free store pickup, with the address fields replaced by the pickup location card.]*
...
Payment itself is a **hybrid of a real gateway integration and a controlled simulation**, switchable at runtime through the owner-facing `Payment Mode` setting (Section 5.10.9). In **Stripe mode**, orders paid by card, FPX online banking, or GrabPay are processed end-to-end through **Stripe Checkout in sandbox (test) mode**: the server creates a Checkout Session for the order's exact total (in Malaysian sen, with the customer's chosen method pre-selected) and redirects the customer to Stripe's hosted, PCI-compliant payment page — the application never sees or stores card details. Because a Stripe session lives for a minimum of thirty minutes, the order's own payment window is stretched to outlive the session (plus a safety margin), so the expiry job can never cancel and restock an order whose hosted session is still payable. E-wallets that Stripe Malaysia does not support (Touch 'n Go, ShopeePay, Boost) remain simulated even in Stripe mode, and **demo mode** returns the entire flow to the original one-click simulation — a deliberate fallback so the system stays fully demonstrable with no network or gateway dependency. The payment page also lets the customer **switch payment method on a pending order** (the choices are whitelisted server-side and any live Stripe session for the old method is expired first), so a method that fails on the gateway's side never strands the order.

Confirmation of a Stripe payment is **never taken from the browser redirect alone**. Two independent server-side paths confirm it: a **webhook endpoint** that cryptographically verifies Stripe's signature before acting (and understands FPX's delayed settlement, where the session completes before the bank confirms), and a **success-URL re-verification** that retrieves the session directly from Stripe's API when the customer returns — which also lets a local demonstration work without any webhook tunnelling. Both paths, along with the demo flow and the owner's manual reconciliation (Section 5.10.3), funnel into the **same atomic, idempotent settle step**, so a replayed webhook, a race between the two confirmation paths, or a double-click can never mark an order paid twice or send a duplicate confirmation email. The same discipline extends to **double-charge prevention on the Stripe side**: an order holds at most one live Checkout session at a time (creation is serialised behind a lock, and any existing session is re-inspected before a new one is minted), a completed-but-still-settling FPX payment shows a self-updating "being confirmed" state instead of offering the customer a second payment, and a pending session whose order total was edited in the meantime is expired and replaced so the stale amount can never be paid. Should a second, different payment nevertheless arrive for an already-settled order, the system detects the mismatched payment reference and immediately alerts the owner to refund the duplicate — a double charge can never sit unnoticed. The same protection is applied from the other direction: every path that takes an order out of play (an administrator cancelling or manually reconciling it, a customer cancelling from their account, the shop being switched to showroom mode) also voids any hosted checkout session still open for that order, and the payment page re-checks that an order is still payable at the moment the customer clicks Pay, so a stale browser tab can never produce a payable session for a cancelled order. A failed asynchronous bank payment detaches its dead session from the order, allowing the customer to simply retry. Finally, a payment that Stripe confirms only after the order was already cancelled (a rare race) is deliberately never "un-cancelled": the stock may already have been resold, so the system instead alerts the owner to refund the charge from the Stripe dashboard, keeping inventory truthful and the money trail explicit.

When payment is confirmed (or manually reconciled by the owner for a bank transfer, Section 5.10.3), the order moves to **processing**, stock is committed, the transition is written to the audit trail with its source (demo, webhook, or return-verification), and the customer receives a confirmation email. The subsequent transitions to shipped (with a tracking number) and delivered are driven by admin actions, each of which emails the customer so they are kept informed at every step.
...
The entire back office is built with **Filament 5** on top of the same Laravel application and database, giving the shop owner a polished, responsive, permission-controlled admin panel without a separate system to maintain. The panel is organised into **fifteen resources**, plus a set of custom actions and pages. Because Filament and the storefront share the same models, a change made in the admin panel is reflected on the storefront immediately.
...
The **Product**, **Category**, and **Brand** resources let the owner manage the catalogue completely. Staff can perform all day-to-day Product management, including direct and bulk updates or deletion; only catalogue export remains administrator-only. Staff can likewise create, edit, enable/disable, reorder, and delete Brands, so the brands displayed in storefront navigation remain current without waiting for an administrator. Categories remain administrator-curated. Products are created and edited with all of their localised fields (Section 5.11), price, stock, category and brand associations, and images (uploaded through the Media Library). The Product table makes stock directly visible and highlights low or zero quantities. On the dashboard, both staff and administrators see an **Inventory Watchlist** showing the ten active products with the lowest stock; zero-stock bars are red and low-stock bars amber, giving the shop floor an immediate replenishment queue without exposing revenue analytics. Categories and brands are managed as their own resources and drive both the storefront navigation and the catalogue filters. Creating a product here is what makes it appear on the homepage and in the catalogue; the storefront has no separate content system.
...
| Mark Shipped / Ready for Pickup | For delivery orders, requires a tracking number and emails the customer the tracking detail; for pickup orders the same action reads "Ready for Pickup", needs no tracking number, and emails the customer to come and collect. |
| Reschedule Pickup | For a non-terminal pickup order, validates a new collection date/time against showroom hours, saves it, and emails the customer the revised collection details. |
...
The **Customer** resource lists registered customers and, because every order carries the customer's `user_id`, shows each customer's order count and lets the owner drill into that customer's orders. The customer is always identified by their user account, which is the reliable link between a person and their purchases. The **Feedback** resource is the homepage testimonial tool, which staff and administrators can create, edit, enable/disable, reorder, or delete. Separately, the **Product Reviews** resource lets both staff and administrators inspect, edit, delete, and directly toggle the storefront visibility of verified-purchase reviews from the listing table. This distinction prevents marketing testimonials from being confused with product-specific customer reviews.
...
| Payment Mode | Whether payment runs simulated ("demo") or through Stripe Checkout in test mode ("stripe") for card / FPX / GrabPay (Section 5.6). |
...
- **Staff (operational worker)** — the day-to-day floor role: opening order details, confirming, editing, and rescheduling bookings, marking orders paid, shipped, and delivered, cancelling orders and processing refunds, importing product and order data, fully managing products, brands, and homepage testimonials (including bulk operations and deletion), editing, reordering, and toggling visibility for the fixed service menu, and working the contact-message inbox. Staff can never **export** data (bulk exfiltration stays admin-only), manage user accounts, create or delete fixed service rows, or delete protected operational records such as orders, bookings, contacts, and categories.
...
**Interface strings** (buttons, labels, navigation, validation messages, and other fixed text) use Laravel's translation system through the `__()` helper, with a key/value file per language (including a translated `validation.php` so form errors appear in the customer's language). Fixed structural content such as category labels is keyed in the same way.

**Owner-editable content** (the parts that the owner writes and that differ per record) is stored in **per-language database columns** (for example a Malay name column and a Chinese name column alongside the default). When a page renders, it selects the column matching the active language and falls back to the default when a translation has not been supplied. Products and installation services both follow this pattern: the owner translates a product's name and description directly in the product form (Section 5.10.2), and a service's name and description in the service form (Section 5.10.3), without any code. Product-review interface strings, pickup date/time labels and validation messages, and the public Statuspage entry are likewise translated through the fixed-interface JSON files, while the System Status chatbot answer is stored in all three languages in its editable database rule. It also means search and sorting operate on the localised text, so a customer browsing in Malay gets a consistent Malay experience.
...
Three further weight reductions target the heaviest assets. The icon library (411 KB) is loaded only by the homepage, the single page that uses it, instead of shipping on every page of the site. The homepage's 3D viewer script (around 1 MB) is injected only when the visitor actually scrolls toward the 3D section, so a visitor who never reaches it never downloads it. The decorative hero background video (around 5.5 MB) respects the visitor first: it is not downloaded at all for users who prefer reduced motion or browse with data-saver enabled, and the hero's gradient layers keep the page presentable without it. Finally, because Livewire's soft navigation re-executes page scripts on every internal page change, all of the layout's global scripts (scroll animations, counters, the scroll-to-top control) register their listeners exactly once and re-attach per navigation only what the new page actually needs — preventing the gradual accumulation of duplicate handlers, and the jank that grows with it, over a long browsing session.
...
The interactive overlays follow a consistent dialog contract. The 3D configurator is a true modal: while closed its controls are removed from the tab order and the accessibility tree entirely, and while open it announces itself as a dialog, moves focus to its close button, traps Tab within itself, closes on Escape, and returns focus to the control that opened it. The cart drawer moves focus in on open and restores it on close, and the chat widget is labelled as the non-modal panel it actually is, so what assistive technology is told always matches what the page does. Selection-style controls announce their state rather than showing it only visually: the booking form's service cards, calendar days, and time slots expose their pressed state (with each calendar day carrying its full localised date), the chat input is programmatically labelled, and the auto-advancing testimonials pause on keyboard focus as well as hover, with an explicit pause/resume button for anyone who wants the movement stopped.

The storefront also honours the visitor's **reduced-motion preference** (the operating-system "reduce motion / minimise animations" setting, exposed to the browser as the `prefers-reduced-motion` media query), addressing WCAG 2.3.3 and helping visitors prone to motion sickness or vestibular discomfort. Every non-essential animation checks this preference and stands down when it is set: the hero background video is not played (or even downloaded), the homepage brand marquee and the testimonial carousel stop auto-scrolling, and the services page's scroll-driven car snaps instantly to position instead of animating along the road. The content itself is never hidden — only the movement is removed — so a visitor who has asked their system to minimise animation still sees the whole site, just without the motion. (A practical consequence worth noting is that on a device with this setting switched on, these animations will legitimately not appear; that is the feature working as intended, not a fault.)
...
The system keeps customers informed through transactional email at every meaningful step: order confirmation, pickup scheduling and pickup rescheduling, shipping with tracking, delivery, cancellation and refund, booking confirmation, and appointment reminders. These are implemented as Laravel Mailables with consistent, branded templates.
...
This chapter has documented the implementation of the Win Win Car Audio Auto Accessories Showroom System from the customer-facing storefront through to the administrative back office and the operational concerns that keep it running in production. The storefront was built as full-page Livewire components on the TALL stack, delivering an interactive homepage, a searchable and shareable product catalogue, an ordered responsive product gallery, verified-purchase product reviews with staff visibility controls, a Three.js 3D configurator, a concurrency-safe cart and checkout with scheduled store pickup, a configuration-driven booking system, public service-status access, and fully trilingual content. The back office was built with Filament across its resources, giving the owner complete, permission-controlled, mobile-friendly control over the catalogue, media, reviews, orders, pickup rescheduling, bookings, services, customers, content, and (through the Settings resource) the shop's own operating parameters, with no code changes required to run the business day to day.

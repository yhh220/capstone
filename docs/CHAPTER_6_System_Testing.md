...
2. **Automated testing**, written with **PHPUnit**, was built alongside the manual testing as a **regression safety net**. It currently comprises **225 tests containing 737 assertions across 46 test files**, and the entire suite passes. Many of these tests exist specifically to *lock down* a bug that had already been found by hand, so that the same defect can never quietly return.
...
- **Array cache/session drivers** and a reduced bcrypt cost keep each run fast: the full 225-test suite completes in under two minutes.
...
The 46 test files are organised by the area of the system they exercise. Table 6.2 shows how the 225 tests are distributed.
...
| Payment & Order Lifecycle | PaymentFlow, PaymentHardening, PaymentExpiryGuard, OrderMarkPaidGuard, OrderCancellation, OrderAdminEdit, OrderTracker, OrderImporter, Invoice, StripeWebhook, StripeCheckoutRedirect, StripeReturnVerification | 75 |
...
| C9 | Check out with "Store Pickup" selected | No address asked for, no shipping fee; "ready for pickup" email received when staff mark it ready | *[to fill]* | |
...
*[Figure 6.13: The full `php artisan test` run showing "Tests: 225 passed (737 assertions)".]*
